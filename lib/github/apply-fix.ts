...
Generated by ${APPLY_FIX_MODEL_LABEL} in agentic mode (${iterations} tool iteration${iterations === 1 ? "" : "s"}). Review the diff before merging — automated edits are not infallible. Disagree with the change? Close the PR and apply the fix manually using the audit recommendations on your CrawlProof project page.
