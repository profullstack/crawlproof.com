  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      <header className="flex flex-wrap items-baseline justify-between gap-4">
        ...
        <Link href={`/dashboard/projects/${projectId}/social/setup`} className="btn">
          Manage accounts
        </Link>
      </header>
      ...
      {accountList.length === 0 ? (
        <section className="card p-5">
          <p className="text-sm text-[var(--color-muted)]">
            ...
            <Link href={`/dashboard/projects/${projectId}/social/setup`} className="underline">
              Connect one
            </Link>{" "}
            to start posting.
          </p>
        ...
        <section className="card p-5">
          ...
          <PostNowForm
            ...
            projectId={projectId}
            urls={(feedItems ?? []).map((f: { url: string; title: string | null }) => ({
              url: f.url,
              title: f.title,
            }))}
          />
          ...
          <ul className="mt-2 divide-y divide-[var(--color-border)] rounded border border-[var(--color-border)]">
            {feedItems!.slice(0, 10).map((item: any) => (
              <li key={item.id} className="px-3 py-2 text-sm">
                ...
                {item.status === "failed" && (
                  <div className="mt-1">
                    <RetryFeedItemButton itemId={item.id} />
                  </div>
                )}
              </li>
            ))}
