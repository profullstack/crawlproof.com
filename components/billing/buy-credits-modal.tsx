    ...
    async (token: Token) => {
      ...
      try {
        ...
        if (data.is_card && data.checkout_url) {
          // success_url/cancel_url were passed to CoinPay, so Stripe will
          // redirect back to our /settings/billing on completion.
  ...
  return (
    <div className="space-y-4">
      <button
        type="button"
        disabled={loading}
        onClick={() => onPick(cardToken)}
        className="flex w-full items-center justify-between rounded-md border border-[var(--color-accent)]/40 bg-[var(--color-accent)]/5 p-3 text-left hover:bg-[var(--color-accent)]/10 disabled:opacity-50"
      >
        <span>
          <span className="block text-sm font-semibold">Credit or debit card</span>
          <span className="block text-xs text-[var(--color-muted)]">
            Visa, Mastercard, Amex · secure Stripe checkout
          </span>
        </span>
