...
try {
  ...
  let ready = false;
  for (let i = 0; i < 60; i++) {
    // Initialization briefly starts a socket-only server, then restarts it.
    // TCP readiness identifies the final server and avoids racing that restart.
    try { execFileSync('docker', ['exec', name, 'pg_isready', '-h', '127.0.0.1', '-U', 'postgres'], { stdio: 'pipe' }); ready = true; break; }
  ...
  execFileSync('docker', ['exec', '-i', name, 'psql', '-h', '127.0.0.1', '-U', 'postgres', '-v', 'ON_ERROR_STOP=1'], { input: fixture.replace('-- APPLY MIGRATION HERE', migration), stdio: ['pipe', 'inherit', 'inherit'] });
